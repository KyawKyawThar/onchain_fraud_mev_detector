...
**Detection accuracy.** Every PR replays ground-truth fixtures through all detectors and fails on a regression below the committed baseline or an active detector below the promotion floor. The committed numbers are precision 1.0 / recall 1.0, over **8 fixtures: one hand-built scenario per detector plus one clean block**. That is a regression signal, not field accuracy. With one negative block, the sample cannot resolve any false-positive rate, so this repository makes no FP-rate claim. Growing the corpus (adversarial negatives, replayed mainnet windows) is tracked as Epic E.
