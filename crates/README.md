...
| [`rebuild`](rebuild/) | **Projection rebuild** (readiness Epic B): the `ReadModel`/`ReplaySource` seams, a canonical row fingerprint, and the one driver that runs `fingerprint → wipe → replay → fingerprint → diff`. §2 says projections are derived; this is how that is *proven* (`just projection-rebuild-drill`) and how a corrupted read model is recovered ([runbook](../docs/runbooks/projection-rebuild.md)). Replays through the event store's published `GET /v1/replay` — never its table — so every envelope crosses the `events::upcast` seam; the owner service implements `ReadModel` (see [`simulation::rebuild`](simulation/src/rebuild.rs)), this crate never touches a store. |
| [`backup`](backup/) | **Backups + tested restore** (readiness Epic B): snapshot/restore/drill for Postgres and ClickHouse (the event store's `events` table included), and the RPO/RTO those drills *measure*. The fingerprint is taken **inside the dump's own cut** (`pg_export_snapshot` → `pg_dump --snapshot`, or the `appended_at < W` watermark), so a restore can be proven row-for-row rather than trusted because a command exited zero; the drill restores into a throwaway database it creates and drops, which is what lets it run on a timer against production ([runbook](../docs/runbooks/backup-restore.md), `just backup-drill`). Tables are discovered, never listed. Deliberately holds no `ch-migrate` edge — a backup must never apply DDL to the database it was called to protect (enforced in `arch-conformance`). Failures are typed transient/permanent/cancelled (`db::is_permanent` for the sqlx half), a `Scratch` database is only constructible by `provision_scratch` and consumed by `drop_scratch` so dropping production does not compile, and leaks are swept by the *next* run rather than by a `Drop` impl no `SIGKILL` would run. |
| [`loadtest`](loadtest/) | **Load test the fast path** (readiness Epic D): offer load at a committed profile (chain tps, alert volume, API qps), drain, and decide whether §6's < 1s p99 held. It exists because the claim was previously unmeasurable — the only latency series was one `detect` call, microseconds on a header-only block and unchanged by a queue in front of it, so the dashboard read green exactly when the claim was most at risk; `detection_fast_path_duration_seconds` was added to span the broker hop and the work queue, and this reads it. Blocks are stamped with the time they were **due**, not sent (coordinated omission); the run **drains** before measuring (the queued blocks are the slow ones); the window is a **difference of two scrapes**; and the verdict is **three-way** — held / breached / *undecided*, because an unloaded system, an undrained pipeline or an empty SLO series all produce a clean-looking p99, and exit `2` must never read as a pass. A bucketed quantile is a bound, so a budget off the ladder is rejected at load rather than turning every run silently inconclusive. Enters through the same doors a customer does — Kafka, HTTP, a `/metrics` scrape — with no store or service edge (enforced in `arch-conformance`) ([runbook](../docs/runbooks/load-test.md), `just load-test`). |
