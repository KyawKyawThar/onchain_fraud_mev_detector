...
| `app-secrets` | `JWT_SECRET`, `EVENT_STORE_WRITE_TOKEN`, `SMTP_USERNAME/PASSWORD`, `ANTHROPIC_API_KEY`, `FEEDBACK_GRANT_SECRET` | api-server, event-store, notification, copilot (JWT_SECRET gates its review API); `FEEDBACK_GRANT_SECRET` goes to exactly two pods — notification mints, api-server verifies (§19 feedback loop) — and must differ from `JWT_SECRET` |
