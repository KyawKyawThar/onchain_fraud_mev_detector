...
- [x] **Event schema evolution strategy (2026-09-02):** [`crates/events/schema/`](crates/events/SCHEMA.md) holds the committed schema of every `DomainEvent` — `v<N>/registry.json`, one directory per `SCHEMA_VERSION`, older ones frozen — plus `corpus/`, the **append-only archive** of every distinct shape ever emitted (including the two genuinely historical pre-§15 shapes). The schema is *probed out of the real `Deserialize` impl* (delete a field and see if it still decodes; read an enum's variants out of serde's own error), not from a second derive that could disagree with the wire format, and it covers the transport contract — topic and partition key — because a re-keyed event changes every consumer's ordering guarantee without touching a field. `crates/events/tests/schema_registry.rs` is the gate — its own CI job, `just schema-check` locally: **any** drift fails the build, classified as compatible (re-commit with `just schema-bless`), consumers-first (a new value in a closed enum — old consumers reject unknown variants, so they deploy first), or breaking (a removed/retyped/newly-required field, a moved topic, a re-keyed partition, a removed event type), which blessing refuses outright. Three checks hold the ends: the whole archive replays through today's reader, every event decodes with unknown fields injected at every level (the forward half that lets a producer ship before its consumers), and each consumer that has opted in (`usage`, `notification`) asserts the fields it reads still exist, so a removal names its own victim. `events::upcast` is the version-dispatch seam `from_json_slice` decodes through — mechanism built and tested, steps legitimately empty at v1, and the first incompatible bump is now a localized change instead of a discovery.
