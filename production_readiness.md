...
**Exit gate:** a full projection rebuild from the event store reproduces current state exactly; restore drill meets RTO. *(Rebuild half: the procedure exists, is proven on the simulation read model, and runs in CI — see above. Not met until intelligence's store is wired to the same seam and the drill has been run once against production-scale data with the duration recorded as the RTO input.)*
