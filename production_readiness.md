...
- [x] **False-positive feedback loop** (2026-09-18): a delivered alert carries a **signed capability** (`crates/feedback-grant`) proving it was shown to that recipient; `POST /v1/incidents/{id}/feedback` verifies it, parks the verdict in a **transactional outbox** (`crates/outbox`, shared with `rule_outbox`) and answers 202 before any broker is involved; the simulation projection folds it into `incident_feedback`, and an exporter publishes the §19 gauges. **The rate divides by adjudicated incidents**, over a *settled* window, **per cohort** — `solicited` verdicts come from incidents the platform picked without looking at the finding, and are the only ones that can back a published claim; `volunteered` ones are self-selected and are a product signal. An unadjudicated window publishes **no rate at all**; the SLO arms on its own gauge and **disarms when one customer dominates the sample** rather than winsorizing a number nobody could reproduce. The disagreement rule and the arming rules are Rust (`feedback_sli`), not SQL: the store returns a fourteen-row contingency table and the policy is a named, unit-tested type. `FalsePositiveFeedbackUnarmed` covers the failure that reads as success — a platform nobody reviews looks exactly like one with no false positives — and `FeedbackSettleTooShort` checks the settle delay against the **measured** verdict lag instead of an assumption. Folding verdicts back into the §20.1 label rule is deliberately **not** done here: `LABEL_RULE_ID` is stamped into every dataset manifest, so changing what a label means is its own reviewed decision.
